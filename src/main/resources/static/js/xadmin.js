;!function (win) {
	...
	Xadmin.prototype.add_tab = function (title,url,is_refresh) {
		...
		for (var i = 0; i <$('.x-iframe').length; i++) {
            if($('.x-iframe').eq(i).attr('tab-id')==id){
                ...
                if(is_refresh){
                    var iframe = $('.x-iframe').eq(i);
                    iframe.attr("src",$('.x-iframe').eq(i).attr('src'));
                    var iframeWindow = iframe[0].contentWindow;
                    var currentHref = iframeWindow.document.location.href.toString();
                    if (currentHref.indexOf("/logout") != -1) {
                        layer.alert("你的账号已在另外一台设备登录，你已被迫下线！",{
                            yes:function(){
  ...
  Xadmin.prototype.del_tab = function (id) {
	...
	Xadmin.prototype.add_lay_tab = function(title,url,id) {
		...
		setTimeout(function () {
		    if($("#iframe_"+id)[0]){
                var iframeWindow = $("#iframe_"+id)[0].contentWindow;
                var currentHref = iframeWindow.document.location.href.toString();
                if (currentHref.indexOf("/logout") != -1) {
                    top.location.href = "/login";
                }
...
var contextPath = 'http://192.168.1.50:8082';
var config = (function($){
    $.path = {
        order:{
            orderListPage: contextPath+'/order/order/selectOrderByPage.json',
            selectOrder: contextPath+"/order/order/selectSingleOrder.json",
            orderDetail: contextPath+"/order/orderDetail/selectOrderDetails.json",
            sendGoods: contextPath+"/order/order/sendGoods.json"
        },
        seller:{},
        product:{
            //查询接口
            query:{
                product:contextPath+'/product/api/product/query.json',
                sysCategory:contextPath+'/product/api/sysCategory/query.json',
                category:contextPath+'/product/api/category/query.json'
            },
            //添加接口
            add:{
                product:contextPath+'/product/api/product/add.json',
                sysCategory:contextPath+'/product/api/sysCategory/add.json',
                category:contextPath+'/product/api/category/add.json'
            },
            //下拉列表
            select:{
                sysCategory:contextPath+'/product/api/sysCategory/select.json',
                category:contextPath+'/product/api/category/select.json'
            },
            list:{
                sysCategory:contextPath+'/product/api/sysCategory/list.json'
            },
            //删除
            delete:{
                sysCategory:contextPath+'/product/api/sysCategory/delete.json'
            }
